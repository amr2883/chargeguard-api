...
const SECRET = process.env.WC_WEBHOOK_SECRET_ALT || process.env.WC_WEBHOOK_SECRET;
if (!SECRET) {
  console.error("خطأ: المفتاح غير موجود في .env.test");
  process.exit(1);
}
...
(async () => {
  console.log("=== Webhook Signature Test (Secure) ===\n");
  const cards = [
