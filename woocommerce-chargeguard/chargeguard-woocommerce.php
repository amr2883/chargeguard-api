...
function chargeguard_init() {
    ...
    if (is_admin() && !file_exists(__DIR__ . '/vendor/stripe/stripe-php/init.php')) {
 ...
 *
 * Only removes drafts older than 2 hours, well past any legitimate
 * in-progress checkout, so a slow real customer is never at risk of
 * having their in-progress order deleted out from under them.
 */
add_action('chargeguard_cleanup_stale_drafts', 'chargeguard_cleanup_stale_drafts');
function chargeguard_cleanup_stale_drafts() {
    ...
    $orders = wc_get_orders([
        ...
        'limit'        => 100,
        'return'       => 'ids',
        // Defense in depth: a blocked order should already have moved to
        // 'cancelled' via ChargeGuard_Dynamic_Firewall::mark_order_blocked()
        // and therefore never match the status filter above — but excluding
        // anything carrying _chargeguard_block_reason here too means this
        // query can never delete a ChargeGuard forensic record even if a
        // future code path leaves one behind in 'checkout-draft' state.
        'meta_query'   => [
            [
                'key'     => '_chargeguard_block_reason',
                'compare' => 'NOT EXISTS',
            ],
        ],
    ]);
    ...
    if (!empty($orders)) {
        error_log('ChargeGuard: cleaned up ' . count($orders) . ' stale checkout-draft order(s).');
    }
}
register_activation_hook(__FILE__, function () {
    if (!wp_next_scheduled('chargeguard_cleanup_stale_drafts')) {
        wp_schedule_event(time(), 'hourly', 'chargeguard_cleanup_stale_drafts');
    }
});
register_deactivation_hook(__FILE__, function () {
...
function chargeguard_missing_stripe_sdk_notice() {
    ...
    <div class="notice notice-warning is-dismissible">
        <p>
            ...
            The Stripe SDK (<code>vendor/stripe/stripe-php</code>) was not found. Stripe webhook enrichment
