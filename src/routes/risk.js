...
router.post('/evaluate', apiKeyAuth, domainAuthMiddleware, async (req, res) => {
  try {
    ...
    if (!merchantId) {
      return res.status(400).json({ error: 'merchantId is required' });
        ...
        // Idempotency: «· Õﬁﬁ „‰ «·ÿ·»«  «·„ﬂ——… Œ·«· ¬Œ— 5 œﬁ«∆ﬁ
    const idempotencyWindow = 5 * 60 * 1000; // 5 œﬁ«∆ﬁ
    ...
    // ≈÷«›… Â–Â «·ﬁÌ„ ≈·Ï computedSignals (Ì„ﬂ‰  „—Ì—Â« ≈·Ï riskScoring ·«Õﬁ«)
    // ·ﬂ‰‰« ”‰” Œœ„Â« „»«‘—… ›Ì signalsSnapshot
    ...
    const response = {
     ...
     // «” Œœ«„ connectedRisk „»«‘—… „‰ ‰ ÌÃ… Identity Graph
  response.connectedRisk = riskResult.graphRisk || 0;
    ...
    if (orderId) {
      ...
      // «” Œœ«„ «·ﬁÌ„ «·„Õ”Ê»… „”»ﬁ« „‰ externalVelocity («·„ÊÃÊœ… ›Ì «·‰ÿ«ﬁ)
      const deviceVelocityCountFinal = deviceVelocityCount;   // „‰ √⁄·Ï «·œ«·…
      const ipVelocityCountFinal = ipVelocityCount;           // „‰ √⁄·Ï «·œ«·…
      const emailVelocityCountFinal = emailVelocityCount;     // „‰ √⁄·Ï «·œ«·…
      ...
      // Õ›Ÿ RiskEvaluation ≈–« ﬂ«‰ «·ﬁ—«— block √Ê review
...
// ‰ﬁÿ… ‰Â«Ì… ·· ⁄·„ „‰ ‰ «∆Ã «·ÕŸ— (Feedback Loop)
//  ” ﬁ»· orderId Ê isFraud (true = «·ÕŸ— ﬂ«‰ ’ÕÌÕ«, false = «·ÕŸ— ﬂ«‰ Œ«ÿ∆«)
...
router.post('/feedback', apiKeyAuth, async (req, res) => {
  try {
    ...
    // «” œ⁄«¡ „Õ—ﬂ «· ⁄·„
    // ‰„—— isFraud ﬂ‰ ÌÃ… (true = lost, false = won)
...
// ‰ﬁÿ… ‰Â«Ì… „ƒﬁ … ·«Œ »«— buildGraphFromOrder
/**
 * @swagger
 * /risk/test-graph:
 *   post:
...
router.get('/blacklist', apiKeyAuth, async (req, res) => {
  try {
    ...
    //  ’›Ì… «·’·«ÕÌ…: »‘ﬂ· «› —«÷Ì ‰⁄—÷ ›ﬁÿ «·⁄‰«’— «·”«—Ì… (€Ì— „‰ ÂÌ… √Ê expiresAt null)
...
router.post('/woocommerce-webhook', async (req, res) => {
  try {
    ...
    // 2. Parse raw body to JSON (‰Õœœ parsedBody «·√Ê·)
    let parsedBody;
    try {
    ...
    // 3. «” Œ—«Ã merchantId (»⁄œÌ‰ ‰” Œœ„ parsedBody)
    ...
    if (wcSecret && signature) {
      ...
      // Secret configured but signature missing ñ reject
   ...
   const existingOrder = await db.order.findUnique({
    ...
    if (existingOrder) {
      ...
      if (ageHours < 24) {
        logger.info({ module: 'risk', orderId: extracted.orderId }, 'Idempotent request ñ returning cached result');
        return res.json({
...
// Ìı” Œœ„ »Ê«”ÿ… Ãœ«— «·Õ„«Ì… «·œÌ‰«„ÌﬂÌ ›Ì «·„ﬂÊ¯‰ «·≈÷«›Ì
router.post('/check-device', apiKeyAuth, domainAuthMiddleware, async (req, res) => {
  try {
    ...
    try {
      ...
      // ≈–« ﬂ«‰ connectedRisk >= 80 ‰⁄ »—Â  ÂœÌœ« ⁄«·Ì« Ê‰„‰⁄Â
    ...
    // ›Ì Õ«·… «·Œÿ√° ‰⁄Ìœ blocked: false ·„‰⁄ «·≈ÌÃ«»Ì«  «·Œ«ÿ∆…
    res.status(500).json({ blocked: false, error: error.message });
  }
});

// ========== Enrich Endpoint ==========
// Ìı” Œœ„ ·≈À—«¡ «·ÿ·» »»Ì«‰«  BIN „‰ »Ê«»«  «·œ›⁄ «·Œ«—ÃÌ… („À· Stripe)
  ...
  try {
    ...
    const updatedSnapshot = {
      ...
      data: {
        ...
        riskLevel: riskResult.riskLevel,
        decision: riskResult.decision.includes('Approve') ? 'approve' : (riskResult.decision.includes('Review') ? 'review' : 'block'),
        cardHash: cardHashRecord?.cardHash ?? null,   // <-- «·”ÿ— «·„÷«›
        signalsSnapshot: JSON.stringify(updatedSnapshot),
      }
    });

    // 9. Õ›Ÿ RiskEvaluation ··ÕœÀ (upsert · Ã‰»  ﬂ—«— orderId)
    await db.riskEvaluation.upsert({
    ...
    // 10. „⁄«·Ã… √Ì pending enrichments (≈–« ÊÃœ  ·Â–« «·ÿ·») - Õ–›Â« ·√‰Â« ÿ»ﬁ 
...
router.post('/tenants/register', async (req, res) => {
  ...
  try {
    ...
    logger.error(
      ...
      'Rate limiter DB error ó failing open'
    );
  }
  // ?? End Rate Limiting ?????????????????????????????????????????????????????
  ...
  try {
    ...
    // «” Œ—«Ã «·œÊ„Ì‰ „‰ storeUrl Ê Œ“Ì‰Â ›Ì allowedDomains
    ...
    // ≈—”«· «·≈Ì„Ì· ó fire-and-forget (·« ÌÊﬁ› «· ”ÃÌ· ·Ê ›‘·)
...
// ========== GET /risk/verify-key ==========
// Ìı” Œœ„ »Ê«”ÿ… “— " Õﬁﬁ „‰ «·„› «Õ" ›Ì ≈÷«›… WooCommerce
// ·« Ì” Œœ„ domainAuthMiddleware ⁄„œ« ó «· «Ã— ﬁœ Ì Õﬁﬁ ﬁ»·  ”ÃÌ· œÊ„Ì‰Â
/**
 * @swagger
 * /risk/verify-key:
 *   get:
...
router.get('/verify-key', async (req, res) => {
  try {
    ...
    // 4. «· Õﬁﬁ «·«Œ Ì«—Ì „‰ «·œÊ„Ì‰
    //    Ìıÿ»Û¯ﬁ ›ﬁÿ ≈–« ﬂ«‰ «· «Ã— ·œÌÂ allowedDomains „”Ã·…
    //    (··Õ›«Ÿ ⁄·Ï «· Ê«›ﬁ „⁄ «· Ã«— «·ﬁœ«„Ï «·–Ì‰ ·Ì” ·œÌÂ„ œÊ„Ì‰ „”Ã·)
    // TODO: ≈÷«›… «· Õﬁﬁ „‰ «·œÊ„Ì‰ ⁄‰œ  Ê›— Õﬁ· allowedDomains
